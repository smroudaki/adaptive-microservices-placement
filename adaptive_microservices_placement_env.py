...
from config import (
    ...
    NUMBER_OF_REQUESTS_MAX,
    REQUESTS_INTERVAL_MIN,
...
class AdaptiveMicroservicesPlacementEnv(MultiAgentEnv):
    def __init__(self, env_config):
        ...
        self._step = 0
        self._current_number_of_requests = NUMBER_OF_REQUESTS_MIN
        self._current_message_interval = REQUESTS_INTERVAL_MAX
    ...
    def render(self, mode="human"):
        ...
        placement_map = self._to_placement_map_log(self._enhanced_placement_map)
        logger.info(f"Placement map: {placement_map}")
    ...
    def _to_placement_map_log(self, enhanced_placement_map):
        """
        Initialize the placement map from the enhanced initial placement, including placement indices.
        ...
        for _, placements in enhanced_placement_map.items():
            for placement in placements:
                _, node_name, ms_id, *_ = placement
                placement_map.append((ms_id, node_name))
    ...
    def _update_user_requests(self):
        ...
        for source in sources:
            ...
            if np.random.rand() < 0.5:
                self._current_number_of_requests = np.random.randint(NUMBER_OF_REQUESTS_MIN, NUMBER_OF_REQUESTS_MAX)
                self._current_message_interval = np.random.randint(REQUESTS_INTERVAL_MIN, REQUESTS_INTERVAL_MAX)

                updated_sources.append(
                    (
                        message_name,
                        allocated_node,
                        self._current_number_of_requests,
                        deterministicDistributionStartPoint(
                            name="Deterministic",
                            start=0,
                            time=self._current_message_interval,
                        ),
                    )
                )
            else:
        ...
        logger.info(f"Current number of requests: {self._current_number_of_requests}")
        logger.info(f"Current message interval: {self._current_message_interval}")
    ...
    def _run_yafs_simulation(self):
        ...
        sim_population = MappedPopulationImproved(
            self._initial_population, name="MappedPopulation"
        )
        ...
        response_time = sim.run(self._current_message_interval * 2)
